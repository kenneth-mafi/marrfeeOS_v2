...
export default function AppDetailsPage() {
  ...
  const handleONGet = async () => {
    if(!(appData && appData.path && appData.id)) return;
    const text = appData?.isInstalled ? "Open" : "Get";
    if (text === "Get") await handleGet(appData.id, text);
    else navigate(appData.path, {replace: true})
  }
    ...
    { Component: AppDetailsTopBar, props: { onBack: goBack }  },
    {
      Component: () => (
        <ScrollArea effect="fade">
          <AppDetailsHeroHeader icon={logoSrc} title={appData.appName} subtitle={`${appData.description.slice(0, 25)}..`} color={appData.color} isInstalled={appData.isInstalled} onGet={handleONGet} />
